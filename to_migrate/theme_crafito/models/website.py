...
class website_editor(models.AbstractModel):
    ...
    def save_asset(self, url, bundle_xmlid, content, file_type):
        ...
        """
        if url != '/theme_crafito/static/src/scss/colors/color_picker.scss':
            return super(website_editor, self).save_asset(url, bundle_xmlid, content, file_type)

        IrAttachment = request.env["ir.attachment"]
        datas = base64.b64encode((content or "\n").encode("utf-8"))
        custom_url = '/theme_crafito/static/src/scss/colors/color_picker' + \
        ...
        if custom_attachment:
            ...
            view_to_xpath = IrUiView.get_related_views(
                "website.assets_frontend", bundles=True).filtered(views_linking_url)
            new_view = {
                'name': custom_url,
...
class website(models.Model):
    ...
    def payment_icons(self):
        ...
        """
        return self.env['payment.icon'].sudo().search([('acquirer_ids.state', 'in', ['enabled', 'test'])])
