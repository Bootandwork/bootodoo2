...
class WebsiteSupportHelpGroup(models.Model):
    ...
    def _page_count(self):
        ...
        """Amount of help pages in a help group"""
            rec.page_count = self.env['website.support.help.page'].search_count([('group_id','=',rec.id)])
...
class WebsiteSupportHelpPage(models.Model):
    ...
    def _compute_feedback_average(self):
        for rec in self:
            ...
            for fb in rec.feedback_ids:
                average += fb.feedback_rating

            if len(rec.feedback_ids) > 0:
                rec.feedback_average = average / len(rec.feedback_ids)
            else:
               rec.feedback_average = 0
