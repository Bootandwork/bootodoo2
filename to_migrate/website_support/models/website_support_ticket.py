...
class WebsiteSupportTicket(models.Model):
    ...
    def _compute_sla_timer_format(self):
        ...
        # Display negative hours in a positive format
            rec.sla_timer_format = '{0:02.0f}:{1:02.0f}'.format(*divmod(abs(rec.sla_timer) * 60, 60))
    ...
    def _compute_planned_time_format(self):
        for rec in self:
            ...
            if rec.planned_time and rec.partner_id and rec.partner_id.lang:
                partner_language = self.env['res.lang'].search([('code','=', rec.partner_id.lang)])[0]

                #If we have timezone information translate the planned date to local time otherwise UTC
                if rec.partner_id.tz:
                    my_planned_time = rec.planned_time.replace(tzinfo=tz.gettz('UTC'))
                    local_time = my_planned_time.astimezone(tz.gettz(rec.partner_id.tz))
                    rec.planned_time_format = local_time.strftime(partner_language.date_format + " " + partner_language.time_format) + " " + rec.partner_id.tz
                else:
                    rec.planned_time_format = rec.planned_time.strftime(partner_language.date_format + " " + partner_language.time_format) + " UTC"

            else:
                rec.planned_time_format = rec.planned_time

    # @api.one
    def _compute_approve_url(self):
        for rec in self:
            rec.approve_url = "/support/approve/" + str(rec.id)

    # @api.one
    def _compute_disapprove_url(self):
        for rec in self:
            rec.disapprove_url = "/support/disapprove/" + str(rec.id)
    ...
    def _compute_unattend(self):
        for rec in self:
            if rec.state_id.unattended == True:
                rec.unattended = True
    ...
    def send_reply(self):
        for rec in self:
            ...
            if rec.approval:
                ...
                rec.ticket_id.state_id = awaiting_approval_state.id

                #One support request per ticket...
                rec.ticket_id.planned_time = rec.planned_time
                rec.ticket_id.approval_message = rec.body
                rec.ticket_id.sla_active = False
            ...
            values = email_wrapper.generate_email([rec.id])[rec.id]
            values['model'] = "website.support.ticket"
            values['res_id'] = rec.ticket_id.id
            send_mail = self.env['mail.mail'].create(values)
            send_mail.send()

            #Add to the message history to keep the data clean from the rest HTML
            self.env['website.support.ticket.message'].create({'ticket_id': rec.ticket_id.id, 'by': 'staff', 'content':rec.body.replace("<p>","").replace("</p>","")})
            ...
            if rec.approval:
                ...
                rec.ticket_id.approval_id = awaiting_approval.id
            else:
                ...
                rec.ticket_id.state_id = staff_replied.id