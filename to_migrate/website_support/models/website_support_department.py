...
class WebsiteSupportDepartment(models.Model):
    ...
    def _compute_sub_category_ids(self):
        for rec in self:
            ...
            for si in self.env['website.support.department.subcategory'].search([('wsd_id', '=', rec.id)]):
                si.unlink()

            extra_access = []
            for extra_permission in rec.partner_ids:
            ...
            for sub_category in self.env['website.support.ticket.subcategory'].search([]):
                ...
                if count > 0:
                    self.env['website.support.department.subcategory'].create( {'wsd_id': rec.id, 'subcategory_id': sub_category.id, 'count': count} )

            rec.sub_category_ids = self.env['website.support.department.subcategory'].search([('wsd_id', '=', rec.id)], limit=5)
    ...
    def _compute_sub_category_month_ids(self):
        for rec in self:
            #Unlink all existing records
            for si in self.env['website.support.department.subcategory'].search([('wsd_month_id', '=', rec.id)]):
                si.unlink()
            ...
            for sub_category in self.env['website.support.ticket.subcategory'].search([]):
                ...
                if count > 0:
                    self.env['website.support.department.subcategory'].create( {'wsd_month_id': rec.id, 'subcategory_id': sub_category.id, 'count': count} )

            rec.sub_category_month_ids = self.env['website.support.department.subcategory'].search([('wsd_month_id', '=', rec.id)], limit=5)
    ...
    def _compute_submit_ticket_contact_ids(self):
        ...
        #Unlink all existing records
        for si in self.env['website.support.department.submit'].search([('wsd_id', '=', rec.id)]):
            si.unlink()

        for contact in rec.partner_ids:
            count = self.env['website.support.ticket'].search_count([('partner_id','=', contact.id)])
            if count > 0:
                self.env['website.support.department.submit'].create( {'wsd_id': rec.id, 'partner_id': contact.id, 'count': count} )

        rec.submit_ticket_contact_ids = self.env['website.support.department.submit'].search([('wsd_id', '=', rec.id)], limit=5)
    ...
    def _compute_submit_ticket_contact_month_ids(self):
        ...
        #Unlink all existing records
        for si in self.env['website.support.department.submit'].search([('wsd_month_id', '=', rec.id)]):
            si.unlink()
        ...
        for contact in rec.partner_ids:
            ...
            if count > 0:
                self.env['website.support.department.submit'].create( {'wsd_month_id': rec.id, 'partner_id': contact.id, 'count': count} )

        rec.submit_ticket_contact_month_ids = self.env['website.support.department.submit'].search([('wsd_month_id', '=', rec.id)], limit=5)
    ...
    def _compute_submit_ticket_contact_week_ids(self):
        for rec in self:
            #Unlink all existing records
            for si in self.env['website.support.department.submit'].search([('wsd_week_id', '=', rec.id)]):
                si.unlink()
            ...
            for contact in rec.partner_ids:
                ...
                if count > 0:
                    self.env['website.support.department.submit'].create( {'wsd_week_id': rec.id, 'partner_id': contact.id, 'count': count} )

            rec.submit_ticket_contact_week_ids = self.env['website.support.department.submit'].search([('wsd_week_id', '=', rec.id)], limit=5)
