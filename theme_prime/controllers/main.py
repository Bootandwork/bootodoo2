...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        if theme_id and theme_id.name.startswith('theme_prime'):
            ...
            tags = []
            if request.website._get_dr_theme_config('json_shop_filters')['show_tags_filter']:
...
class ThemeWebsite(Website):
    ...
    def dr_search(self, term, max_nb_chars, options, **kw):
        ...
        if search_config.get('search_category') and not has_formulate:
            ...
            # Search from category synonyms if no category found
            if results.get('categories') and not results.get('categories').get('results_count'):
                category_result = self.autocomplete(search_type='category_synonyms', term=term, order='name asc', limit=5, options=options)
                results['categories'] = {'results': category_result['results'], 'results_count': len(category_result['results']), 'parts': {}}
    ...
    def _match_category(self, term=False, categories_ids=[]):
        CSD = request.env['product.public.category']._get_search_db(request.website)
        return CSD.search(term=term, categories_ids=categories_ids)
